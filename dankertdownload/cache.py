...
class Cache:
    ...
    def _get_root_dir(self):
        ...
        if res is None:
            ...
            res = os.path.join(cache_root, 'dankert-download')
    ...
    def store(self, section, key, data, dtype='json'):
        ...
        try:
            ...
            write_json_file({'dankert-download_version': __version__, 'data': data}, fn)
        except Exception:
            tb = traceback.format_exc()
            self._ydl.report_warning(f'Writing cache to {fn!r} failed: {tb}')

    def _validate(self, data, min_ver):
        version = traverse_obj(data, 'dankert-download_version')
