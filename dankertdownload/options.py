...
def parseOpts(overrideArguments=None, ignore_config_files='if_override'):  # noqa: N803
    ...
    def add_config(label, path=None, func=None):
        ...
        if root.parse_known_args()[0].ignoreconfig:
            ...
            current_path = os.path.join(path, 'dankert-download.conf')
...
class _YoutubeDLOptionParser(optparse.OptionParser):
    ...
    def __init__(self):
        super().__init__(
            prog='dankert-download' if detect_variant() == 'source' else None,
            version=__version__,
            usage='%prog [OPTIONS] URL [URL...]',
            epilog='See full documentation at  https://github.com/dankert-download/dankert-download#readme',
    ...
    def format_option_help(self, formatter=None):
        ...
        description = formatter.format_description(
            'Predefined aliases for convenience and ease of use. Note that future versions of dankert-download '
...
def create_parser():
    ...
    general.add_option(
        ...
        help=(
            ...
            'Use the value "auto" to let dankert-download guess ("auto_warning" to emit a warning when guessing). '
    ...
    general.add_option(
        ...
        callback_kwargs={
            ...
            'configurations by reverting some of the changes made in dankert-download. '
    ...
    authentication.add_option(
        ...
        help='Account password. If this option is left out, dankert-download will ask interactively')
    authentication.add_option(
        '-2', '--twofactor',
    ...
    authentication.add_option(
        ...
        help='Multiple-system operator account password. If this option is left out, dankert-download will ask interactively')
    authentication.add_option(
        '--ap-list-mso',
    ...
    authentication.add_option(
        ...
        help='Password for client certificate private key, if encrypted. '
             'If not provided, and the key is encrypted, dankert-download will ask interactively')
    ...
    verbosity.add_option(
        ...
        # help='Contact the dankert-download server for debugging')
        help=optparse.SUPPRESS_HELP)
    verbosity.add_option(
        '--no-call-home',
        dest='call_home', action='store_false',
        # help='Do not contact the dankert-download server for debugging (default)')
        help=optparse.SUPPRESS_HELP)
    ...
    filesystem.add_option(
        ...
        help=(
            'Location in the filesystem where dankert-download can store some downloaded information '
            '(such as client ids and signatures) permanently. By default ${XDG_CACHE_HOME}/dankert-download'))
    filesystem.add_option(
...
def _hide_login_info(opts):
    deprecation_warning(f'"{__name__}._hide_login_info" is deprecated and may be removed '
                        'in a future version. Use "dankert_install.utils.Config.hide_login_info" instead')
