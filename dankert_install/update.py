...
def _get_variant_and_executable_path():
    ...
    if getattr(sys, 'frozen', False):
        ...
        if not hasattr(sys, '_MEIPASS'):
            ...
            # See: https://github.com/dankert-download/dankert-download/issues/11813
...
_NON_UPDATEABLE_REASONS = {
    ...
    **{variant: f'Auto-update is not supported for unpackaged {name} executable; Re-download the latest release'
    ...
    'py2exe': 'py2exe is no longer supported by dankert-download; This executable cannot be updated',
    'source': 'You cannot update when running from source code; Use git to pull the latest changes',
    'unknown': 'You installed dankert-download from a manual build or with a package manager; Use that to update',
    'other': 'You are using an unofficial build of dankert-download; Build the executable again',
}
...
def _get_binary_name():
    return format_field(_FILE_SUFFIXES, detect_variant(), template='dankert-download%s', ignore=None, default=None)
...
class Updater:
    ...
    def __init__(self, ydl, target: str | None = None):
        ...
        if '/' in self.requested_channel:
            ...
            if not self.requested_repo.startswith('dankert-download/') and self.requested_repo != self._origin:
    ...
    def _process_update_spec(self, lockfile: str, resolved_tag: str):
        ...
        for line in lines:
            ...
            if re.match(pattern, self._identifier):
                ...
                self._report_error(
                    f'dankert-download cannot be updated to {resolved_tag} since you are on an older Python version '
    ...
    def query_update(self, *, _output=False) -> UpdateInfo | None:
        ...
        if not has_update:
            if _output:
                self.ydl.to_screen(f'{latest_or_requested}\ndankert-download is up to date ({current_label})')
            return None
    ...
    def update(self, update_info=NO_DEFAULT):
        """Update dankert-download executable to the latest version
        ...
        self.ydl.to_screen(f'Updated dankert-download to {update_label}')
        return True
    ...
    def cmd(self):
        ...
        if getattr(sys, 'orig_argv', None):
            ...
            argv = sys.argv
        # linux_static exe's argv[0] will be /tmp/staticx-NNNN/dankert-download_linux if we don't fixup here
    ...
    def _block_restart(self, msg):
        def wrapper():
            self._report_error(f'{msg}. Restart dankert-download to use the updated version', expected=True)
...
def run_update(ydl):
    ...
    """
    deprecation_warning(
        '"dankert_install.update.run_update(ydl)" is deprecated and may be removed in a future version. '
        'Use "dankert_install.update.Updater(ydl).update()" instead')
